...
def train(exp_config: str = './configs/seld.yml',
    ...
    for val_perm in range(1, 6):
        # Load data module
        datamodule = build_datamodule(cfg=cfg, feature_db=feature_db)
        datamodule.set_val_perm(val_perm)
        ...
        # Console logger
        console_logger = MyLoggingCallback(val_perm)
        ...
        submission_dir = os.path.join(cfg.dir.output_dir.submission, '_temp{}'.format(val_perm))  # to temporarily store val output
